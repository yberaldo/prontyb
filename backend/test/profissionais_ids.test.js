...
test('buscarPorId rejeita id "1abc" com 400', async () => {
  const controllerPath = path.resolve(__dirname, '..', 'src', 'controladores', 'profissionais.controlador.js');
...
test('ativar rejeita id "1abc" com 400', async () => {
  const controllerPath = path.resolve(__dirname, '..', 'src', 'controladores', 'profissionais.controlador.js');
  clearCacheIfExists(controllerPath);
  const controller = require(controllerPath);
  ...
  await controller.ativar(request, reply);

  assert.equal(reply._code, 400);
  assert.equal(reply._body && reply._body.mensagem, 'id invalido');
});

test('desativar rejeita id "1abc" com 400', async () => {
  const controllerPath = path.resolve(__dirname, '..', 'src', 'controladores', 'profissionais.controlador.js');
  clearCacheIfExists(controllerPath);
  const controller = require(controllerPath);
  ...
  await controller.desativar(request, reply);
...
test('buscarPorId com id "1" chama servico e retorna sucesso', async () => {
  const servicePath = path.resolve(__dirname, '..', 'src', 'servicos', 'profissionais.servico.js');
  const controllerPath = path.resolve(__dirname, '..', 'src', 'controladores', 'profissionais.controlador.js');
